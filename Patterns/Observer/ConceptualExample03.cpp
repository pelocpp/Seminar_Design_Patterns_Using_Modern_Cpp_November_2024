...
namespace ObserverDesignPatternSmartPointerEx {
    ...
    public:
        ...
        Observer(std::shared_ptr<Subject> subject) : m_subject{ subject } 
        {
            ...
            // NEIN - bin doch gerade im Konstruktor der beteiligten Klasse 
        ...
        void removeMeFromTheList() 
        {
            if (m_subject != nullptr) {
                try {
                    std::shared_ptr<Observer> tmp{ shared_from_this() };

                    // bräuchte jetzt eine shared_ptr von mir selbst:
                    // std::shared_ptr<Observer> tmp (this);

                    m_subject->detach(tmp);
